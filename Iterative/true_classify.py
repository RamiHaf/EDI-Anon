...
def test_images_classification(original_model, device, local_test_loader, excel_file_path, save_roc_dir):
    ...
    for data, target in tqdm(local_test_loader):
        ...
        logits.append(output)
    final_acc = correct / float(len(local_test_loader))
    print('the number of correct images')
    print(correct)
    ...
    f1 = f1_score(all_labels.cpu(), predicted_labels.cpu(), average='micro')
    precision = precision_score(all_labels.cpu(), predicted_labels.cpu(), average='micro')
    recall = recall_score(all_labels.cpu(), predicted_labels.cpu(), average='micro')
